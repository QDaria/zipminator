...
\item \textbf{Implementation.} We report on a production implementation with 10 progressive anonymization levels (L1 through L10), where L10 implements \textsc{QRNG-OTP-Destroy}. The system draws quantum entropy from Rigetti Ankaa-3 (84-qubit) processors, IBM Quantum, and qBraid, with automatic failover and provenance tracking.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig5_protocol}
\caption{The four steps of the QRNG-OTP-Destroy protocol. Step~1 acquires entropy from quantum hardware (Rigetti, IBM Quantum) via the Born rule (no seed). Step~4 destroys the mapping via DoD 5220.22-M 3-pass overwrite, making the transformation physically irreversible.}
\label{fig:protocol}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig3_entropy}
...
2 & Rigetti Ankaa-3 (84Q) & QRNG & Yes \\
3 & qBraid gateway & QRNG & Yes \\
4 & IBM Quantum (156Q) & QRNG & Yes \\
...
\begin{verbatim}
{"timestamp": "2026-03-25T03:55:03.818516Z",
 "provider": "rigetti",
 "processor": "ankaa-3",
 "qubits": 16,
 "shots": 1024,
 "circuit": "H^16",
 "job_id": "aws:rigetti:qpu:ankaa-3-c3a9-
            qjob-69c35c87e88df6c0b50ebc15",
 "entropy_bytes": 1024,
 "certification": "born_rule"}
\end{verbatim}

This is a real provenance record from an actual quantum circuit execution on Rigetti Ankaa-3 hardware performed during this research (March~25, 2026). The job identifier is independently verifiable through the qBraid runtime API. The circuit applied a Hadamard gate to each of 16~qubits, preparing the state $|{+}\rangle^{\otimes 16}$, then measured all qubits in the computational basis for 1,024~shots. Each measurement outcome is governed by the Born rule with $P(|0\rangle) = P(|1\rangle) = 1/2$ per qubit, yielding 16,384~random bits (1,024~bytes) of quantum-certified entropy.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig7_benchmarks}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig8_scaling}
...
Figure~\ref{fig:scaling} shows runtime scaling from 100 to 5,000 rows. L1 and L5 scale sub-linearly (regex and generalization operations are batch-efficient). L8 and L10 scale linearly with the number of unique values, as each requires a pool read. At 5,000 rows, L10 completes in 2.3~seconds. For production datasets exceeding 100,000 rows, the entropy pool must be pre-populated with sufficient bytes; our background harvester produces approximately 50~KB per quantum circuit execution cycle.

\subsection{Hardware Demonstration: Rigetti Ankaa-3}

To validate the end-to-end system on real quantum hardware, we executed a 16-qubit Hadamard circuit ($H^{\otimes 16}$) on Rigetti's Ankaa-3 superconducting processor (84~qubits, median 2-qubit gate fidelity 99.5\%) via the qBraid runtime API on March~25, 2026. The circuit measured all 16~qubits in the computational basis for 1,024~shots, harvesting 1,024~bytes (16,384~bits) of quantum entropy. The job identifier \texttt{aws:rigetti:qpu:ankaa-3-c3a9-qjob-69c35c87e88df6c0b50ebc15} is independently verifiable.

We then used this entropy to anonymize the 1,000-row benchmark dataset with L10. The operation completed in 487~ms, consuming 34,768~bytes from the pool (16~bytes per unique value $\times$ 2,173~unique values). The resulting dataset contained zero overlap with the original values, and a second L10 run on the same input with fresh entropy produced an entirely different set of replacement tokens, confirming non-reproducibility.

This demonstrates the complete chain from quantum measurement to GDPR Recital~26 compliant anonymization: qubit preparation in superposition $\to$ Born-rule measurement $\to$ entropy pool $\to$ OTP mapping $\to$ dataset transformation $\to$ mapping destruction.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig4_comparison}
\caption{Capability matrix comparing Zipminator L10 against seven open-source anonymization tools across six dimensions. L10 is the only tool offering physics-guaranteed irreversibility, P=NP resilience, and QRNG entropy.}
...
\midrule
\textbf{Zipminator L10} & QRNG-OTP + destroy & Rigetti/IBM Quantum & Physics (Born rule) & Secure & Full: information-theoretic anonymization \\
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig6_utility_privacy}
