% main-anonymous.tex — Paper 2: Unilateral WiFi CSI as NIST-Validated Entropy Source
...
\documentclass[sigconf,nonacm=false,anonymous]{acmart}
...
%% ─── Custom commands ───
\newcommand{\zipminator}{\textsc{[SystemName]}\xspace}
...
\copyrightyear{2026}
\acmYear{2026}
\acmConference[CCS '26]{ACM Conference on Computer and Communications Security}{October 2026}{TBD}
...
\input{figures/fig5-bilateral-vs-unilateral}
...
\input{figures/fig6-xor-composition}
...
\input{figures/fig2-puek-protocol}
...
\input{figures/fig3-minentropy-bars}
...
\input{figures/fig9-extraction-sensitivity}
...
\input{figures/fig4-cost-benefit}
...
An entropy system that honestly pauses during low-quality conditions is strictly preferable to one that silently outputs low-entropy bytes; the variance monitor is a feature, not a limitation.

Figure~\ref{fig:static-degradation} illustrates this degradation pattern and the variance monitoring response.

\input{figures/fig7-static-degradation}
...
\input{figures/fig8-correlation-heatmap}
...
Integration of ARE conditioning into the CSI extraction pipeline is the subject of ongoing work.
...
The extraction pipeline, Von~Neumann debiaser, pool architecture, PUEK enrollment code, and XOR compositor are open-source components of the \zipminator project, available at \texttt{[anonymous repository]}.
...
All source code for the extraction pipeline (Python and Rust), NIST SP~800-90B assessment scripts, PUEK enrollment and verification, and the XOR compositor are available in the open-source repository \texttt{[anonymous repository]}.
