...
impl Kyber768 {
    ...
    pub fn encapsulate_with_coins(
        ...
        g_input.extend_from_slice(coins);      // m (message/coins)
        g_input.extend_from_slice(&pk_hash);   // H(pk)
        ...
        for i in 0..KYBER_K {
            let mut nonce = randomness.to_vec();
            nonce.push(i as u8);
        ...
        ss_input.extend_from_slice(&shared_key);  // K (shared secret seed)
        ss_input.extend_from_slice(&ct_hash);     // H(c)
    ...
    pub fn decapsulate(ct: &Ciphertext, sk: &SecretKey) -> SharedSecret {
        ...
        pk_hash.copy_from_slice(&sk.data[pk_hash_offset..pk_hash_offset + KYBER_SYMBYTES]);

        let z_offset = pk_hash_offset + KYBER_SYMBYTES;
        let implicit_rejection = &sk.data[z_offset..z_offset + KYBER_SYMBYTES];
        ...
        for i in 0..KYBER_N {
            ...
            // Using the formula from FIPS 203: bit = ⌊(2*diff + Q/2) / Q⌋ mod 2
            let t = ((2 * diff + KYBER_Q as i32 / 2) / KYBER_Q as i32) & 1;
            msg[i / 8] |= (t as u8) << (i % 8);
        }

        // Re-derive: (K', r') = G(m' || H(pk)) and re-encapsulate
        // Note: encapsulate_with_coins will compute G(m' || H(pk)) internally
        ...
        let mut valid_ss_input = Vec::new();
        valid_ss_input.extend_from_slice(&k_prime);
        valid_ss_input.extend_from_slice(&ct_hash);
        let valid_ss = sha3_256(&valid_ss_input);

        // Invalid case (implicit rejection): K_bar = H(z || H(c))
        let mut invalid_ss_input = Vec::new();
        invalid_ss_input.extend_from_slice(implicit_rejection);
        invalid_ss_input.extend_from_slice(&ct_hash);
