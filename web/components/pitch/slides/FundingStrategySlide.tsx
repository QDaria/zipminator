...
export default function FundingStrategySlide({ scenario: _scenario = 'base' }: { scenario?: Scenario }) {
  ...
  return (
    <SlideWrapper>
      ...
      {/* Grant Amounts Chart -- MOVED UP to upper half */}
      <motion.div {...fadeUp(0.04)} className="card-quantum mb-6">
        ...
        <ResponsiveContainer width="100%" height={280}>
          <BarChart layout="vertical" data={GRANT_AMOUNTS_NUMERIC} margin={{ left: 10, right: 40 }}>
            ...
            <Legend
              ...
              height={24}
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              formatter={(value: any) => <span style={{ color: '#d1d5db', fontSize: 11 }}>{value}</span>}
            />
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mt-3">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs font-mono text-gray-500 uppercase tracking-widest">Total Potential:</span>
            <span ref={totalRef as React.RefObject<HTMLSpanElement>} className="text-2xl font-bold gradient-text font-mono">${animatedTotal}</span>
          </div>
        </div>
      </motion.div>

      {/* 3-Tier Visual Timeline */}
      <motion.div {...fadeUp(0.08)} className="grid sm:grid-cols-3 gap-4 mb-8">
        {[
          {
            icon: Landmark,
            tier: 'Tier 1',
      ...
      <motion.div {...fadeUp(0.08)} className="mb-8">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h3 className="text-base font-semibold text-white">Norwegian Quantum Ecosystem</h3>
