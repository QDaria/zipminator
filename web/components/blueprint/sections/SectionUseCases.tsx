...
export const SectionUseCases = () => {
  const prose = SECTION_PROSE['use-cases']
  // Subsections A-C render between chart and cards; D-F render after cards
  const subsectionsBeforeCards = prose?.subsections.filter(
    (s) => s.id === 'intelligence-defense' || s.id === 'financial-services' || s.id === 'healthcare'
  ) ?? []
  const subsectionsAfterCards = prose?.subsections.filter(
    (s) => s.id === 'telecom-iot' || s.id === 'enterprise-saas' || s.id === 'urgency-rationale'
  ) ?? []
  ...
  return (
    <div className="space-y-10">
      ...
      {/* --- Subsections A-C (after chart, before cards) --- */}
      {subsectionsBeforeCards.length > 0 && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
        ...
        >
          {subsectionsBeforeCards.map((sub) => (
      ...
      {/* --- Subsections D-F (after cards) --- */}
      {subsectionsAfterCards.length > 0 && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
