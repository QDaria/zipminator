...
export const SectionIPScoring = () => {
  const prose = SECTION_PROSE['ip-scoring']
  ...
  return (
    <div className="space-y-12">
      ...
      {/* Subsection A: Scoring Methodology */}
      {prose?.subsections?.[0] && (
      ...
      {/* Subsections B-D: Per-patent scoring (P1, P2, P3) */}
      {prose?.subsections?.slice(1, 4).map((sub) => (
      ...
      {/* Subsections E-F: Platform scoring + Portfolio synergy */}
      {prose?.subsections?.slice(4).map((sub) => (
        <Subsection key={sub.id} heading={sub.heading}>
          <ProseBlock paragraphs={sub.body} />
          {sub.callout && (
            <CalloutBlock
      ...
      {/* Conclusion prose */}
      {prose?.conclusion && <ProseBlock paragraphs={prose.conclusion} />}
    </div>
  )
}
